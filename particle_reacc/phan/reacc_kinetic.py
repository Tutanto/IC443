...
def reacc_kinetic(ene, co, br, s, particle):
    ...
    def f(x): return ((np.sqrt(x*(x+2*rest))/cc.value)**(alpha-1)) * \
        (x**i1 * (1+x/E_br)**(-i2))/(beta(x)*cc.value)
    ...
    n_acc_1 = an_1[0] * (alpha+2) * (p2k(ene_1))**(-alpha) * \
        np.exp(-(p2k(ene_1))/co) * dp_dk(ene_1) * const
    n_acc_2 = an_2[0] * (alpha+2) * (p2k(ene_2))**(-alpha) * br / \
        (p2k(ene_2)) * np.exp(-(p2k(ene_2))/co) * dp_dk(ene_2) * const
    ...
    if particle == 'proton':
        n_fin = s**(2/3) * np.concatenate((n_acc_1.value,
                                           np.maximum(n_p.value[idx:], n_acc_2.value)))
    if particle == 'electron':
        n_fin = s**(2/3) * np.concatenate((n_acc_1.value,
                                           np.maximum(n_e.value[idx:], n_acc_2.value)))
    return n_fin
